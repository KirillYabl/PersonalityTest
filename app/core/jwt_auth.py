...
def create_access_token(user: UserIdOut) -> Token:
    ...
    encoded_jwt = jwt.encode(
        to_encode, settings.APP_SECRET_KEY.get_secret_value(), algorithm=settings.ACCESS_TOKEN_ALGORITHM
    )
...
async def get_current_user(
    ...
    try:
        payload = jwt.decode(
            token, settings.APP_SECRET_KEY.get_secret_value(), algorithms=[settings.ACCESS_TOKEN_ALGORITHM]
        )
