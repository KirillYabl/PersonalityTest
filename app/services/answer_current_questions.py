...
from core.errors import InvalidAnswersException, QuizAttemptNotFoundException
from core.errors_base import ServiceExceptionGroup
from db.database import get_db_session, transaction
from db.repositories import QuestionAnswerRepository, QuestionRepository, QuizAttemptRepository
...
async def _validate_answers(
    answers: list[_CurrentQuestionAnswerDTO], question_repository: SQLAlchemyRepository
) -> list[list[str]]:
    ...
    """
    filters = (Question.uuid.in_({answer.question_uuid for answer in answers}),)
    ...
    for i, answer in enumerate(answers):
        ...
        match question.answer_type:
            case "integer":
                if (
                    type(answer.value) != int  # noqa: E721
                ):  # строгое сравнение, т.к. там хранится json и например isinstance(True, int) == True
                    suberrors.append(
...
async def _delete_old_answers(
    ...
    answer_questions: list[QuestionAnswerIdOut] = await question_answer_repository.get_many(
        *filters, out_data=QuestionAnswerIdOut
    )
...
class AnswerCurrentQuestionsService:
    ...
    async def __call__(self, user_uuid: UUID, quiz_uuid: UUID, answers: list[BaseModel]) -> SQLAlchemyOutModel:
        _exceptions_group = self._exceptions_group_class(
            "Ошибки в сервисе AnswerCurrentQuestionsService", [ValueError()]
        )
