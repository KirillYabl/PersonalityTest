...
from sqlalchemy import BinaryExpression, BooleanClauseList, delete, desc, insert, select, update
...
class BaseRepository(ABC):
    ...
    @abstractmethod
    async def update_same_many(
        self, ids: Iterable[Id], in_data: InData, out_data: OutData, **kwargs
    ) -> Iterable[OutData]:
...
class SQLAlchemyRepository(BaseRepository):
    ...
    async def create_many(
        self, in_datas: Iterable[Mapping[str, Any]], out_data: SQLAlchemyOutModel | None, **kwargs
    ) -> Iterable[OutData]:
        async with get_db_session() as session:
            async with transaction(session=session):
                if out_data is None:
    ...
    async def update_one(
        self, id: Id, in_data: BaseModel, out_data: SQLAlchemyOutModel | None, **kwargs
    ) -> SQLAlchemyOutModel | None:
        stmt = (
            update(self.model).filter(getattr(self.model, self.primary_key_name) == id).values(**in_data.model_dump())
        )
        async with get_db_session() as session:
            async with transaction(session=session):
        ...
        async with get_db_session() as session:
            ...
            for relationships in out_data.relationships:
                for relationship in relationships:
                    relationship_models[relationship.property.mapper.class_.__name__] = (
            ...
            if order_by:
                ...
                for column_name in order_by:
                    ...
                    if sorting_field is None:
                        logger.warning(
                            f"Не удалось сделать сортировку по {column_name} т.к. в модели не заданы правила сортировки"
                        )
                        continue
            ...
            if out_data.relationships:
                for sub_relationships in out_data.relationships:
                    joins = [
                        [
                            relationship_models[sub_relationships[0].property.mapper.class_.__name__],
                            sub_relationships[0],
                        ]
                    ]
                    ...
                    for sub_relationship in sub_relationships[1:]:
                        ...
                        joins.append(
                            [relationship_models[sub_relationship.property.mapper.class_.__name__], sub_relationship]
                        )
